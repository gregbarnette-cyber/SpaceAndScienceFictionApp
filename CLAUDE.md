...
**`pytest` is the runner.** The tests are *written* as `unittest.TestCase` classes — pytest collects them natively, so that is a style choice, not a second runner to maintain (`pytest.ini` sets `testpaths = tests`). Current state: **2685 passed, 3 skipped, 494 subtests, 0 failures** (2026-08-02, after Phases AP/AQ/AR). The 3 skips are the opt-in real-dust-map tests (`SPACE_APP_RUN_HEAVY_DUST=1` to run them) — a routine `pytest -q` no longer loads the multi-GB dust cube, so it stays memory-safe on the 8 GB WSL box (loading it mid-sweep OOM-crashed WSL 2026-08-02; see the `heavy_dust_enabled` gate in `tests/_dustcheck.py`). Always invoke through the venv (`venv/bin/python -m pytest`); a bare `pytest` may resolve to a system install on system Python without this project's deps. *(The stdlib `unittest discover` still happens to work, since nothing uses pytest-only features — but it is not verified against and no doc should promise parity; don't spend a run on it.)*

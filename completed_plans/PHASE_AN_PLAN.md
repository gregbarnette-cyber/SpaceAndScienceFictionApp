...
| **D4** | **Opt-50 rebuild: defer or gate?** (§5, AN2c) | **AN2** | **(a)** Defer — lookup surfaces get the names now, DB-backed surfaces catch up at the next rebuild. **(b)** Gate the phase on a full opt-50 rebuild (17 SIMBAD queries, ~238k rows, ~~hours~~ **10–15 min — maintainer, 2026-07-29; the cost input to this decision was overstated, see the T2 note**) | **(a) defer.** A stale `designations` column is already an accepted state in this repo (the NAME-first change left the same debt). **Note:** the rebuild changes **row count**, not just column text — re-measure the `PLX …` discard delta when it happens |
...
| **T2** | **Anyone runs option 50** (CLI or GUI), for any reason | Event | D4 discharges itself. **Record `count` + `discarded` before and after** and re-measure the AN2c `PLX …` delta. Update this section and `docs/star-databases.md` with the measured numbers in the same commit. **Before-numbers captured 2026-07-29: 256,003 rows · 0 `PLX ` main_ids · 0 rows carrying a Bayer/Flamsteed token** (38 rows match a naive `'%* %'` LIKE, but they are `NAME GR* 215A`-style ids, not Bayer). **The rebuild costs ~10–15 min, not the "hours" D4's option (b) assumed** — the cost input to the deferral was overstated, so if the asymmetry is ever mildly annoying, just run it. ~~Sequencing caveat (after D5)~~ retired by [A6]. **D8 closed first (2026-07-29), so a rebuild now freezes rule-based picks rather than SIMBAD-ordering ones** — that ordering was luck, not planning, and is worth preserving if this ever reruns on a different branch |
...
(43 stars, later 47 — see the D8 close in §4b) → `tests/test_designation_harness.py` +
`tests/fixtures/designation_golden.json`.
...
| **AN4.0** | **Capture the fixture corpus (live SIMBAD)** | Low | ✅ **BUILT 2026-07-29** — `tests/_capture_designation_fixtures.py` → `tests/fixtures/designation_ids.json`, 43 stars / 27 Bayer / 20 Flamsteed / 35 `**` / 14 `V*` ids. **Grown to 47** by the D8 close (4 tie shapes the original corpus could not express), via a new `--only-new` mode so a re-capture cannot mix upstream drift into an intentional addition |

...
- **DB columns**: `star_name`, `designations`, `spectral_type`, `parallax`, `parsecs`, `light_years`, `app_magnitude`, `ra`, `dec`. These are also the column order written by opt 51's CSV export (`Star Name, Star Designations, Spectral Type, Parallax, Parsecs, Light Years, Apparent Magnitude, RA, DEC`).
  - Star Name: `main_id`; Star Designations: comma-separated IDs — SIMBAD's common **NAME** first, then **Bayer, Flamsteed** (Phase AN2), then the catalog IDs (GJ, HD, HIP, HR, Wolf, LHS, BD, K2, Kepler, KOI, TOI, CoRoT, COCONUTS, HAT_P, WASP, TIC, Gaia EDR3, 2MASS) — parsed from the pipe-separated `ids.ids` string via `_parse_designations_from_ids()`. Output order follows the key list, so a named star reads `NAME Chara, GJ 475, HD 109358, HIP 61317, …` (* bet CVn).
    - **Bayer/Flamsteed are NOT in the stored column yet (Phase AN2 D4 — deferred).** The *builder* emits them from the moment AN2 landed, but this column is only written by an option-50 run, and no rebuild has happened since. So a SIMBAD-lookup surface (opts 1, 3–6, 8–10) shows the new designations while a `star_systems`-backed one (opts 18/19, opt 51's CSV, the Route Planning tables, the G1 `designation_prefix` search) does not. **That asymmetry is the accepted state, not a parser bug** — the identical debt the NAME-first change left above. It discharges itself the next time anyone runs option 50; when that happens, record `count` + `discarded` before and after, because adding keys makes `desig_str` non-empty for stars that captured nothing before, so the `PLX …` discard rule keeps rows it used to drop and the **row count** can move, not just the column text (live sampling says the practical delta is nil — re-measure, don't assume). Full trigger list in `PHASE_AN_PLAN.md` §5 AN2c-T.

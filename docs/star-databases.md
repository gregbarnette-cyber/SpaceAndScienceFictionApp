...
- **`gcns.main` rows:** `distance_method = "gcns_bayesian"`, full Gaia photometry (`phot_g/bp/rp_mean_mag`), `wd_prob`, Bayesian `dist_pc`/`dist_lo_pc`/`dist_hi_pc`, and (**CR-20**) Gaia proper motion `pmra`/`pmdec`/`pmra_error`/`pmdec_error`/`ruwe`.
- **CR-20 (Gaia PM backbone, 2026-09-03):** `gcns_stars` persists `pmra, pmdec, pmra_error, pmdec_error, ruwe` from the `gcns.main` query (added to `_GCNS_MAIN_ADQL` + the insert; the 5 columns are ALTER-migrated at the end of `gcns_stars` via `_migrate_schema`). **Schema-additive only** — they are **NOT** in `_GCNS_ROW_COLS`, so no GCNS reader (`gcns-source`/`gcns-system`/`gcns-within-sol`/`gcns-stars-within-star`/`dossier`) output changes; PM is a backbone for CR-21, not surfaced in CR-20. `missing_10mas` rows have no PM in source → **null**, never fabricated. Population without a destructive re-ingest is `core.databases.backfill_gcns_proper_motion()` — a targeted `ALTER` + PM-only `UPDATE` by `source_id` that touches only the 5 PM columns (existing `gcns_stars` values + the resolved-system tables byte-identical). The DB is gitignored/machine-local — the backfill fulfils it for one box; another gains PM only via a full opt-58 re-ingest under the extended ADQL. See `docs/integration.md` (CR-20 block).
...
- **GCNS Distance / Travel Time / Stars Within a Star** (`GcnsDistancePanel` M4a, `GcnsTravelTimePanel` M4b, `GcnsStarsWithinStarPanel` M4c) → `compute_gcns_distance` / `compute_gcns_travel_time` / `compute_gcns_stars_within_star`. M4c keeps Gaia-resolved close companions (excluded only by exact source_id) and reuses the opt-18/19 **Star Chart** + **Star Chart 3D** tabs with the center gold-highlighted.
  - **CR-18 (2026-09-01) — transverse separation + bound flag.** `compute_gcns_stars_within_star` (and the `query.py gcns-stars-within-star` reader) now attach per-neighbour `transverse_sep_au`/`transverse_sep_ly` (GCNS `proj_sep_au` for a co-systemed neighbour, else angular-sep × the centre's GCNS distance), a tri-state `bound` (`true`/`false`/**`null`=outside the GCNS layer, NOT "unbound"**), `is_bound_companion`, `sep_method`, and `radial_parallax_dominated` (error-aware on this path — `gcns_stars` carries `parallax_error`). The bound flag comes from the centre's incident `gcns_system_pairs` (the shared `databases.gcns_center_pair_map`); the synthetic Sol row carries all new keys (null/False). Additive — the GUI panel/tables are unchanged. See `docs/integration.md` (CR-18 block).
  - **M4c synthesizes Sol (2026-07-31).** Gaia does not observe the Sun, so `gcns_stars` has no Sol row and never will — the same gap opt 19 had against `star_systems` (see `docs/calculators.md`, opt 19). `compute_gcns_stars_within_star` appends one at the origin when the center is within `limit_ly`, using the standard `_GCNS_ROW_COLS` shape with **every field it cannot honestly fill left `NULL`** — no `gaia_source_id`, no Bayesian distance, no Gaia G/BP/RP — plus `star_name="Sol"`, `spectral_type="G2V"`, `app_magnitude=-26.74`, `dist_pc`/`light_years`/`x`/`y`/`z` = 0. It is flagged **`in_gcns = False`** and **`distance_method = "synthetic_sol_origin"`** (rendered **"Synthetic (origin)"** — `gcns.py::_meth` falls back to the *raw* key, so a new method without a label entry would surface the identifier at the user), so the row can never be read as catalogue astrometry and either field filters it out. This keeps the panel's headline promise honest: Sol has no Bayesian distance and shows a `—` uncertainty, like a `missing_10mas` row. Self-exclusion is the existing `1e-9` floor, and `compute_gcns_within_sol` (M1) is untouched — its distances are measured *from* Sol. On the two chart tabs Sol is drawn as a **★** rather than a dot — `_gcns_map_stars` sets the `is_sol` flag off `distance_method` (not the display name), which `plot_helpers._overlay_sol_marker` paints; see `docs/calculators.md` (opt 19) for why it is an overlay artist and not a marker swap.

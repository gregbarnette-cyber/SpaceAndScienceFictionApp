...
**Total: 182 subcommands** (distinct `query.py` `sub.add_parser` names — 182 calls, no aliases; all 182 are
...
| `exclusion-boundary` | FTL exclusion-boundary radius r_ex (the "Alcubierre Limit"); a Kuiper-calibrated in-universe dial. CR-22: a **two-layer** output — the unchanged canon **standoff** + a second **research-grade physical WALL** (`wall_au`/`wall_route`/`wall_exceeds_standoff`, wind flags `--wind-speed`/`--v-ism`/`--c-ms`/`--b-field`/…); a four-value `domain` enum (`main_sequence`/`evolved`/`windless_free_harbor`/`unmodeled`) with a windless free-harbor guard on every path + evolved-host standoff from a measured mass (`--star-mass-catalog`). CR-23: the `--star` mass chain is the **dossier tier ladder** (manual > catalog > Gaia FLAME > L-inversion — a non-cataloged FLAME star now matches `dossier`/`exclusion-system`, e.g. ε Eri ≈43.69); every output carries **`mass_provenance`** ∈ {manual,catalog,gaia_flame,ms_luminosity_inversion,object_preset,spectral_type_table} (or null on a no-mass domain); a bounded FLAME degrade surfaces **`flame_status`**; **`--gaia-timeout`** now accepted. CR-25: `--wind-state {quiet,solar,active,hot}` is a full bin override on any main-sequence colour (precedence: `--object` preset `wind_class` > `--wind-state` > active-otype auto-detect > colour default; ignored + noted on evolved/windless/unmodeled hosts); on `--star` an MS K/M star's **full** SIMBAD otype list is fetched (one bounded TAP call) → `active` iff any of `{Er*, Fl*, UV*, BY*, RS*}`; new keys on every output **`wind_class_provenance`** ∈ {manual, otype_auto, class_default, object_preset}/null + `wind_otype` + `wind_otype_source` + `wind_class_note`; a degraded otype fetch surfaces top-level **`otype_status`** ∈ {timeout, unreachable, error}. Standoff unchanged (γ=0); an active K/M wall 0.424 → 13.42 AU. CR-22.6: `--mass-msun` must be finite and > 0 again (≤ 0/NaN → "must be > 0", +inf → "must be finite", exit 1 — a CR-22 regression fix). |
| `exclusion-system` | CR-11.3: compose `exclusion-boundary` over a binary/multi-star system (`--star` or `--component`) into merge-grouped, phase-varying, asymmetric zones with per-component off-MS domain guards. CR-19: `--gaia-timeout` bounds the Gaia-archive TAP calls; a `--star` degrade flags `gaia_status` (binary path) + `flame_status_a`/`_b` (mass path), and a mass-resolving `--component` degrade flags per-component `flame_status_<a+i>`, never hangs. CR-22: the two-layer wall lands per component + a parallel top-level `wall_zones[]` (envelope + combined-wind on summed Ẇ + phase-eligibility); the four-value `domain` enum; per-`--component` wind keys + system-level wind flags. CR-23: per-component **`mass_note`** (parity with `exclusion-boundary`) + **`standoff_note`** (research-grade for an evolved component; null for MS/windless); the single-body `--star` FLAME degrade now surfaces `flame_status` too. CR-25.4: new system-level **`--wind-state`** (reaches MS components without their own `wind_state=`); per-`--component` `wind_state=` honored for all four values on any MS colour (`otype=` accepts a `|` list; `--component` never fetches); `--star` components carry their otype + (MS K/M) the full SIMBAD otype list; per-component `wind_class_provenance`/`wind_otype`/`wind_otype_source`/`wind_class_note` + resolved `mass_loss_msun_yr`/`mass_loss_provenance`; degrade flags `otype_status` (single body) / `otype_status_a`/`_b` (binary). |

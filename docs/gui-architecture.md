...
| `StarComparisonPanel` | — (GUI-only, Phase L1) | `panels/comparison.py` |
...
| L (L1–L3) | Complete | **Exoplanet Comparison Dashboard** (GUI; one `query.py` subcommand): new "Comparison" nav category — `StarComparisonPanel` (L1), `EsiRankingPanel` (L2), `StellarEvolutionPanel` (L3) in `panels/comparison.py`. L1 `core/databases.compute_stars` fans SIMBAD + NASA-supplement + HZ + Hypatia per star (per-star error isolation) → transposed table + grouped abundance-comparison chart (`core.viz.prepare_abundance_comparison` → `make_abundance_comparison_canvas`). L2 is **presentation-only over the Phase G2 `search_hwc`** (no new core fn, no `esi-ranking` subcommand). L3 `core/equations.compute_stellar_evolution` (self-validating) backs the panel + the `stellar-evolution` query.py subcommand + the timeline diagram (`core.viz.prepare_evolution_diagram` → `make_evolution_canvas`). **L4 (Hypatia cache + abundance search) is deferred** behind a verification spike (bulk `/data` path + rate-limit/IP-block check) — see `PHASE_L_PLAN.md`. See `docs/equations.md`, `docs/star-databases.md` (Phase L), `docs/integration.md`. Tests: `tests/test_comparison.py`. |

...
**Per-platform font normalization (2026-07-31).** The **Windows 8 pt look is the target on every platform**, so `gui_main.py::_apply_app_font(app)` sets it explicitly: Windows gets `QFont("Segoe UI", _WINDOWS_FONT_POINT_SIZE)`; Linux/WSL keeps the **default family** and sets only the point size (`_LINUX_FONT_POINT_SIZE`). Both are 8. Linux was previously left at the Qt default (Sans Serif 9 pt → Noto Sans, 12 px @96 DPI), which rendered noticeably larger than Windows (Segoe UI 8 pt, ~10.7 px @96 DPI) — the repo is developed on both, so the two had visibly different densities. The family is deliberately *not* forced on Linux: Segoe UI is not installed there, so overriding it would fall back unpredictably; Noto Sans 8 pt (11 px) is the closest available match. Set `_LINUX_FONT_POINT_SIZE = None` to restore the old Linux default. **No panel sets a Qt point size** (verified 2026-07-31 — `gui_main.py` holds the only two `setPointSize`/`QFont(` call sites in the GUI), so every widget inherits the application font; that is what makes this single knob effective, and hard-coding a size would opt that widget out of it. Matplotlib's in-figure `fontsize=` values in `plot_helpers.py` are a separate axis — they are canvas text, not Qt widgets, and this font does not reach them.
...
gui/                 # Qt presentation layer
  ...
  panels/
    ...
    honorverse.py         # HonorverseHyperPanel (14), HonorverseAccelPanel (15),
                          #   HonorverseSpeedPanel (16); Phase K: HonorverseHyperTimePanel,
    ...
    catalogs.py          # HwcPanel (6), OecPanel (7 — OEC tree + detail pane + per-host Hypatia/diagrams)
    oec_detail.py        # OEC System View: the per-tag field registry, `detail_model` (pure) and
                         #   `build_detail_pane`/`host_band_model` (Qt). Owns the shared OEC value
                         #   formatters — catalogs.py imports them, so the tree and the pane cannot
                         #   render one field two ways (the reverse import would be circular).
    diagram_tabs.py      # shared stateless diagram-tab builders (_make_{hz,orbits,mass_radius,transit,size}_tab)
                         ...
                         #   Also the shared Star-Chart builders (_build_iso_chart_tab / _build_star_chart_3d_tab) +
                         #   the O15 row↔map link helpers, moved here from distance_stars.py so the other panels can
                         #   reuse the exact opt-18/19 builders without a circular panel import. Used by the two-star
                         #   panels (opts 17/20/21) AND — since the route-chart refactor 2026-07-27 — all seven Route
                         #   Planning panels, via route_planning.{add_two_star_chart_tabs,_add_route_chart_tabs}.
                         #   _build_iso_chart_tab(..., label_max_ly=None, routes=None, legend_filter=True): routes= is
                         #   the Phase-I overlay passthrough; legend_filter=False is JumpNetworkPanel's opt-out.
    nasa_exoplanet.py    # NasaPlanetarySystemsPanel (3), NasaHwoExepPanel (4),
                         #   NasaMissionExocatPanel (5), NasaPlanetarySystemsMapPanel (GUI-only)
    ...
    route_planning.py    # Phase I: MultiStopJourneyPanel (I1), NearestNeighborPanel (I2),
                         #   TradeRoutePlannerPanel (I3, stretch); Phase I-OPTS: OptimalTourPanel (A),
                         #   JumpRoutePanel (B), JumpNetworkPanel (C), FarthestFirstPanel (D).
                         #   Both Star-Chart tab builders live here — _add_route_chart_tabs (the seven
                         #   planners) and add_two_star_chart_tabs (opts 17/20/21) — and both go through
                         #   diagram_tabs._build_iso_chart_tab (route overlay via its routes= passthrough).
                         #   Each panel also carries a DESCRIPTION class attr shown by the Show/Hide
                         #   Description button (a persistent label atop the results pane).
    ...
    solvent_zones.py     # Phase P: SolventZonePanel, IceLineCalculatorPanel, SolventReferencePanel
    projects.py          # Phase S: ProjectPanel (project workspaces)
    wikipedia_tab.py     # shared Wikipedia article tab: WikipediaView, WikipediaButtonMixin,
                         #   open_or_focus_wiki_tab (used by six star-facing panels)
...
- Left pane: `QTreeWidget` navigation (280–480 px inside a resizable `QSplitter`, initial width 300 px)
...
moving the `(category, [...])` tuples — no test pins it (the four tests — in three files — that read `NAVIGATION` use
...
| `DbStatusPanel` | 57 (GUI only) | `panels/csv_utility.py` — DB-table row counts + the cached dust-map files (`core.dust.get_dust_map_status()`) + the research-priors cache status (`core.research_priors.get_research_priors_status()`, Phase R3; the schema cell appends `(+v2: <blocks>)` when Phase R3-V2 superset blocks are loaded, from the status dict's `v2_blocks` list) |
...
| `ImportResearchPriorsPanel` | — (GUI-only, Phase R3 — research-priors contract import; the status readout + post-import summary add a "v2 superset blocks: …" line when the dataset carries Phase R3-V2 blocks) | `panels/csv_utility.py` |
...
> **OEC System View — star-first master/detail (built 2026-08-02, `completed_plans/OEC_SYSTEM_VIEW_PLAN.md`).**
> The Data tab of `OecPanel` is a **`QSplitter` of a 10-column tree + a detail pane**, plus a
> toolbar. New files: `gui/panels/oec_detail.py` (the field registry, section builders and the
> shared OEC value formatters) and `core/oec_derived.py` (pure derived values, no Qt, no I/O).
> - **Columns** `Node · Type · M · R · P (d) · a (AU) · e · T (K) · L / S⊕ · HZ` — the last two carry a star's derived luminosity + HZ range and a planet's insolation + HZ verdict —
>   `_oec_tree_cells` builds them per tag, so **star rows populate M/R/T in the same columns as
>   planets** and both carry derived cells. Node column is **Interactive at 300 px** (Stretch
>   starved it to ~95 px beside the pane); the numeric columns are `ResizeToContents`, and
>   **Hide empty columns** drops any column no row populates.
> - **Selection is one attribute** (`_oec_sel`), fed by four selectors — the tree
>   (`selectionModel().currentChanged`, **not** `itemClicked`, so arrow keys work), a map star /
>   ◆ click, the Host combo, and the pinned band. `_set_oec_selection` renders the pane **and
>   nothing else**: a selection change must never rebuild the viz tabs. The programmatic tree
>   sync uses a re-entrancy flag (`_oec_syncing`) — `tree.blockSignals()` does not block
>   `currentChanged` (it belongs to the selection model) and `selectionModel().blockSignals()`
>   also suppresses the view's own painting slots.
> - **The pane lives INSIDE the Data tab**, never as a sibling tab: `_rebuild_after_focus`
>   deletes every tab above index 0, so a pane-as-tab would be destroyed on each recenter.
> - **Detail pane** for all five tags — a star gets the eight-block dossier (Identity · Position
>   & distance · Physical · Photometry · Habitable zone & ice lines · Planets hosted · Companions
>   & hierarchy · Cross-references), and every tag ends with an alphabetical **"Other catalogued
>   fields"** fallback so a field OEC adds tomorrow cannot go missing. Each section builds inside
>   its own `log_viz_error` guard. `_show_oec_planet_dialog` (the Architecture map's click-planet
>   dialog) renders from the **same** builder with the panel's live ctx.
> - **Derived values are violet, badged ◇, tooltipped with their `source`, and never merged into
>   the catalogue rows**; a value that cannot be computed still renders, carrying its reason.
> - **Pinned host band** — selecting a planet swaps the pane, so the host's L / HZ / snow line /
>   distance stay pinned above it (`host_band_model`); clicking it selects the host. Absent for
>   non-planet selections and for rogue planets. A circumbinary planet pins the **pair**, using
>   the same `_oec_host_of` resolver its own derived rows use.
> - **Toolbar** — Units (`Auto / M⊕ R⊕ / M♃ R♃`, catalogued planet mass+radius only, Auto decided
>   per node), Detail pane (Right / Below / Hidden), Errors, Derived, Hide empty columns, Pin host
>   star. Session-only (no `QSettings`); a toggle **re-texts the existing tree items**
>   (`_oec_refresh_tree_cells`) rather than rebuilding the tree, so expansion, scroll position and
...
>
> **Retained deliberately — do not delete without checking with the owner (verified 2026-08-02).**
> All three have **zero instantiation or import sites** anywhere in the repo: the only reference to each
> class name is its own file plus the `_VIZ_PANEL_MODULES` map entry. That is a *measurement, not a
> verdict* — the docs record their status ("standalone stub (not in nav)") but have never recorded why
> they were **kept** rather than removed, so absence of call sites must not be read as absence of intent.
> Three things to know before touching them:
> - **A `grep` for the class name is exactly how you conclude they are dead — and it is the wrong tool.**
>   They are reached through a module-level `__getattr__`, so `from gui.panels import SystemOrbitsPanel`
>   resolves at runtime with no static reference anywhere. Any future consumer would be invisible to grep.
> - **The lazy `__getattr__` is load-bearing and unrelated to their disuse.** These panels inherit from
>   `gui.panels.base`, so importing them eagerly in `gui/panels/__init__.py` re-enters that package before
>   it finishes initialising. The indirection is a real circular-import fix, not vestigial scaffolding.
> - **`system_orbits.py` carries a live bug that was deliberately NOT fixed**: it plots planet orbits at
>   `zorder 3` under a data-space star `Circle(radius=max_au * 0.015)` at `zorder 10` — the same defect
>   corrected in `make_orbits_canvas` on 2026-08-02, which there buried Mercury on the opt-11 Planets
>   diagram. It is unreachable from the UI, so no user can hit it; **if this panel is ever wired into the
>   nav, port the screen-space marker fix first.** `hz_diagram.py` shares the `max_au * 0.018` pattern but
>   draws no orbits, so it occludes nothing.

## Search & Filter Panels (Phase G — `panels/search.py`, `panels/search_common.py`)

The three Phase-G **Search & Filter** nav entries (`StarSystemsSearchPanel`,
`HwcSearchPanel`, `NasaExoplanetSearchPanel`) introduce an **inline drill-down
tab** pattern (Phase L4's `HypatiaSearchPanel` later reused `SearchPanelBase`, without
`SpectralClassControl`) — new vs. the rest of the GUI, which shows one panel at a time.
...
A `Find star: [____] [Find] [Clear]` row inserted at index 1 of `_viz_container`'s layout
(between the "Show Tables" button row and the tabs widget), so it sits above every map tab at
once. Substring match on **name OR designations**, whitespace-collapsed + case-folded
(`_norm_find`, so `61 Cyg A` matches the stored `*  61 Cyg A`); repeating the same query cycles
the matches, a new query restarts at the first; no match → `"No match"` + a status-bar line and
**no view change**.

**It lives in `gui/panels/diagram_tabs.py`** (`_norm_find`, `_dedupe_find_rows`,
`_find_rows_from_table`, `_find_on_map`, `_clear_find`, `_add_find_box`) as of the Route-Find
change, 2026-07-31 — moved out of `distance_stars.py`, which imports `_add_find_box` and re-exports `_norm_find` / `_find_on_map` / `_clear_find` (the names `O18FindTest` imports) (that
module imports `route_planning`, so the reverse import would be circular; the re-export also
keeps `tests/test_viz_phase_o.py`'s imports working). Four things to know:

- **The searchable set is `panel._find_rows` — a `(name, designations)` list — not the result
  table.** That is what makes it work on the four leg-shaped route panels, whose tables are
  `From|To` legs with no per-star row and no `link_view` at all. Route panels source it from the
  `_centered(rm)` star dicts; opts 18/19 source it from the table via `_link_name_col` (0 there,
  and the column after it for designations). Table-sourced panels set `_find_rows_live` and
  **re-derive on every find** rather than using the render-time snapshot: `make_table` enables
  sorting and `QStandardItemModel.sort` physically reorders rows, so a snapshot would cycle matches
  in render order while the user is looking at a sorted table (the old live scan followed the sort).
  Route panels never set the flag — **including the three that do render sortable per-star tables**
  (Nearest-Neighbor, Farthest-First, Jump Network). Their searchable set is the route star list by
  design, which is what covers the four leg-shaped panels, and re-deriving it from the table would
  change the *set*, not just its order — Jump Network's table carries the start (`tiers[0]`), which
  is deliberately excluded. After a user sorts one of those three tables the ring therefore cycles
  in route order (hop / step / tier) while the row selection scrolls non-monotonically; route order
  is the more meaningful cycle on a route panel, so that is the intended trade. The one remaining
  difference from the old behaviour: two table rows sharing a Star Name now collapse to one entry
  instead of reading "1 of 2" and cycling — which was already a lie, since the coord map is
  name-keyed and both cycle steps centred the same dot. It is
  **deduped by name, first occurrence wins** — Multi-Stop and Optimal Tour emit one node per
  typed stop, so a revisited star appears twice while the canvases' name-keyed coord maps
  collapse it to one dot; without dedupe a find would read "1 of 2" and centre twice on it.
- **`_find_on_map` rings the canvases directly** (`canvas.highlight_star(name)`), and `_clear_find`
  clears them directly. Previously the ring was purely a side effect of table selection
  (`_star_click_select` → `selectionChanged` → `_on_link_selection`), which no `link_view=None`
  panel has — so the leg-shaped panels would have panned with no ring. Table selection is still
  performed when a table *is* linked (the O15 gesture), but it is an extra, not the mechanism.
- **The start star (`stars[0]`, the gold ★) is excluded on all seven route panels**, so the
  gesture has one outcome everywhere — including opts 18/19, where the centre star has no table
  row at all. The O16 hazard is the motivation: the ★ is drawn outside the per-class scatter but
  **is** in the ring's `name_cls` map, so filtering its class off would suppress its ring while
  the ★ stayed visible, and the find's reveal step would then un-hide a class the user
  deliberately filtered off. Excluded **by name, not by index**, so a route that returns to its
  start (`Sol → Sirius → Sol`) does not smuggle it back in at a later index. **Neither premise
  holds for `JumpNetworkPanel`** — its start *does* have a table row (`tiers[0]`) and it passes
  `legend_filter=False`, so nothing is ever hidden — meaning typing the start's name there reports
  "No match" for a star visibly in the table and in the canvas coord map. Accepted deliberately:
  making it the one panel where the start is findable would give the same gesture two outcomes.
- **Lifecycle.** `_add_find_box` adds nothing when the searchable set is empty, so an empty
  Nearest-Neighbor/Farthest-First chain and Jump Route's `reachable=False` chart get no box — the
  same "nothing to find" reasoning that keeps it off opts 17/20/21. Since the waypoint work
  (2026-07-31) Jump Route passes `find_box=False` **only when that failed chart has no waypoints**:
  with `via` stars it carries k+2 terminals, not two endpoints, so the box is warranted. `_prepare_render` (`base.py`) clears `_find_rows`/`_find_matches`/`_find_idx`
  **and hides the box**, because every panel renders twice per Run and every error path returns
  *before* the chart tabs are built. Hiding is load-bearing, not tidiness: `_viz_container` and
  `_find_widget` both outlive a render, so a result that builds charts but no box — a reachable
  Jump Route followed by an unreachable one — would otherwise leave the previous run's box on
  screen with its old query, over an empty searchable set (`_add_find_box`'s `existing.show()` is
  what brings it back). `_build_iso_chart_tab`'s isochrone `_rebuild` also clears the cycle state **and the
  readout**: it swaps the canvas for one with a fresh `view0`, so a stale `_find_idx` would make
  the next identical Find *advance* the cycle instead of re-centring, a later Clear would silently
  fail to restore the view, and a leftover "2 of 3 matches — …" label would be describing a state
  that no longer exists. The query text itself survives — it is still what the user wants. `_add_find_box` treats a `_find_widget` whose C++ object was freed by
  `reset()` as stale and rebuilds (touching it raises `RuntimeError`, which would abort `_render`
  before `_finish_render` and leave "Show Diagrams" hidden). **`JumpNetworkPanel` gets no legend
  reveal** — it passes `legend_filter=False`, so `_o16_reveal_class` does not exist and no class
  is ever hidden; the `getattr` guard covers it. **Known limit (pre-existing, opts 18/19 only):** a table row whose map star has
  `x is None` is dropped by `core.viz.prepare_star_map_from_result` but still enters `_find_rows`,
  so it is findable with no dot — `highlight_star` records the name and draws nothing, `center_on`
  returns `False`. The old table scan behaved identically. It cannot arise on the route panels,
...
| `make_orbits_canvas(parent, orbits, hz_zones, max_au, star_name, eeid_au, markers, solar_overlay=False, title=None, km_axis=False, hyper_au=None, snow_au=None, solvent_bands=None)` | NASA opts 3, 6, Map panel; Solar System (11) — Phase O O7 | Keplerian orbital ellipses with HZ annulus overlay. **Phase O O4** `solar_overlay=True` (additive): dashed grey reference circles at the Solar planets' SMAs (`core.viz._PLANET_SMAS`) that fit `max_au×1.1`, with end-of-orbit labels (drawn under the planet orbits); default `False` → byte-identical to before. Driven by the `wrap_orbits_with_solar_toggle` checkbox. **Phase O O7** (additive): `title` overrides the "Planetary Orbits" diagram title; `km_axis=True` adds a secondary top x-axis in km (AU × 1.496e8) for moon-system diagrams; both default off → byte-identical. **Phase O O10b** (additive): `hyper_au` > 0 draws a dashed-red Honorverse hyper-limit ring at that AU (expanding the frame to fit it); default `None` → byte-identical. **Phase P V6** (additive): `snow_au` > 0 draws a dashed-cyan water-snow-line ring (M2). **Phase P V7** (additive): `solvent_bands` (list of `{name, inner_au, outer_au, color}`) shades translucent solvent annuli behind the orbits; both default `None`/empty → byte-identical (the V6/V7 checkboxes default off via `wrap_orbits_with_solar_toggle`'s `snow_au`/`solvent_options` params, fed by `core.viz.prepare_orbit_overlays`). **Scroll-wheel zoom (2026-08-02, applies to every caller):** the canvas now connects a `scroll_event` handler that zooms around the cursor, and `toolbar.push_current()` seeds the nav stack so **Home** restores the initial frame afterwards (scroll bypasses the toolbar, so without the seed Home had no view to return to). Both axes are scaled by the same factor, preserving `aspect="equal"` — a single-axis zoom would draw circular orbits as ellipses. Previously only the toolbar's rectangle-zoom existed, which is far too coarse for diagrams spanning ~3 orders of magnitude (Sedna's ~1180 AU apastron beside a 1 AU orbit). Safe because no caller wraps this canvas in `wrap_scrollable` (which swallows wheel events). Pinned by `tests/test_viz_phase_o.py::OrbitsCanvasScrollZoomTest`. **Star marker fix (2026-08-02):** the star was `Circle(radius=max_au * 0.015)` — sized as a fraction of the frame, in **data space**. On the Solar-System planets diagram (max_au ≈ 38 AU) that made the Sun a **0.569 AU** blob that (a) completely buried Mercury's orbit (apo 0.466 AU, drawn at zorder 3 under the star's zorder 10) and (b) put the `star_name` label at `star_r × 1.8` = **1.025 AU — on top of Earth's orbit**. Both scaled *with* the orbits, so zooming could never uncover them. It is now a fixed **screen**-size marker (`markersize=9`) with the label as an `annotate(..., textcoords="offset points")` pinned to the origin. The old "Not shown (inside star circle at this scale)" note was removed with it — nothing is permanently hidden now. **Do not reintroduce a data-space star radius**; `tests/test_viz_phase_o.py::OrbitsStarMarkerTest` fails if you do. Note `make_hz_canvas` (`plot_helpers.py`), `hz_diagram.py` and `system_orbits.py` still carry the same `max_au * 0.015`/`0.018` pattern — lower impact there (no planet orbits to occlude), but the same trap. **Per-orbit labels (2026-08-02):** bodies were named only in the legend; each orbit now also carries an on-plot `annotate` label anchored to its **topmost vertex** with a fixed pixel offset (so concentric orbits stack up the +y axis in radial order and each label stays glued to its curve at any zoom). Visibility is zoom-driven via `xlim_changed`/`ylim_changed`, gated on **how many label anchors are in frame** vs `_ORBIT_LABEL_MAX_IN_VIEW` (40) — all shown at or under it, all hidden above. **This is deliberately NOT the opt-18/19 rule.** Those gate on an absolute 15 ly visible half-range, which cannot transfer: these diagrams span four orders of magnitude (a moon system ~0.003 AU across, the dwarf/asteroid view ~1180 AU), so any fixed AU threshold is always-on or always-off. Two rejected alternatives, both measured: a **radial-band** test (`[peri, apo]` overlapping the visible annulus) never resolves a dense belt, because the ~259 asteroids are overlapping *eccentric* rings and every annulus is crossed by 100+ of them; and `peri <= r_max` alone additionally counts every tighter orbit inside the view. Anchors are single points, so they separate under zoom, and the test matches what `annotation_clip` already draws. Measured: opt-11 Planets (8), Moon Systems (<=16) and Dwarf Planets + Asteroids **(major)** (39) label immediately; **(all)** (264) stays unlabelled until ~0.5 AU across, which is honest — 259 names cannot share one frame. Pinned by `tests/test_viz_phase_o.py::OrbitsLabelVisibilityTest`. |
...
> **Spectral-class colour & legend bucketing (2026-07-27 — `completed_plans/SPECTRAL_CLASS_PLAN.md` Part 2).** Every dot colour and
> per-class legend entry now derives its class letter through `core.shared.spectral_leading_class(sp, _SP_DISPLAY_LETTERS)`,
> surfaced as `core.viz._sp_color` (colour) and `gui.visualizations.plot_helpers._display_class` (legend / label /
> highlight bucketing). Previously a naive `sp[:1].upper()` painted `dM6` (Wolf 359) **white-dwarf blue** and filed it
> under a bogus "Class D" legend entry, while `sdM3.0` → `S` and `esdL7` → `E` fell through to grey.
> **All 17 display sites must use these helpers and be converted together** — the legend loops, `name_cls`
> (highlight suppression, `_attach_highlight_2d/3d`) and `label_groups` (`_apply_labels`) agree only by producing the
> same string, so a partial conversion breaks legend filtering *silently* (no error, no test failure); pinned by
> `test_search.py::SpectralDisplayClassTest::test_cross_site_agreement`. The display alphabet is the search chips
> **plus** `L T Y W D C N`; `R`/`S` are deliberately excluded (zero catalogue rows, and they made the non-spectral
> label `"Red Giant"` resolve to carbon class R). `core.calculators._star_map_color` (route maps + opts 17/20/21)
> was a **second palette** disagreeing on G/M/D and the unknown grey; `completed_plans/ROUTE_CHART_REFACTOR_PLAN.md` **Phase 3
> deleted it (2026-07-27)**. The palette + lookup now live in **`core/shared.py`** (`_SPECTRAL_COLORS` + `sp_color`),
> beside the `spectral_leading_class` rule they key off; `core.viz` re-exports them under the historical
> `_SPECTRAL_COLORS`/`_sp_color` names, so the 17 display sites were untouched. There is now exactly one palette
> app-wide — pinned by `test_search.py::test_there_is_exactly_one_spectral_palette`. `_SPECTRAL_COLORS` is physically motivated and deliberately fails
> the generic categorical-palette checks (F ↔ G at OKLab ΔE 2.7 — both really are near-white); identity is carried by
> legend labels / hover / click-info, never hue alone.
>
> **Unknown-type bucket is filterable (2026-08-14).** A blank/unparseable `sp_type` buckets to `"?"`. It used to be
> drawn but left off the pickable legend (the O-3 "unfilterable, always visible" decision), so a blank-type star (e.g.
> **Wolf 424 A/B**) survived opt 18 with every class de-selected. Both `_legend_filter_2d`/`_legend_filter_3d` now emit
> it as an **"Unknown"** legend entry, **shown last**, so "clear all" empties the chart; the two **map** canvases'
> non-interactive (`legend_filter=False`) static legend was made to match (Unknown last) so the two legend paths can't
> drift. Pinned by `test_viz_phase_o.py::O16LegendFilterTest` (2D) / `O16LegendFilter3DTest` (3D).
...
| `make_star_chart_canvas(parent, stars, limit_ly, routes=None)` | Stars Within Distance 18, 19; Route Planning (Phase I); the O8 two-star maps 17, 20, 21 | Labeled X–Y star chart in the dark navy palette of `generate_star_map_html.py`; no title; scaled grid/major-tick/distance-ring intervals; per-star `"Name (Z=±X.XXX)"` labels anchored with a **fixed pixel offset** (`annotate(textcoords="offset points")`, so labels track their dots on zoom rather than drifting) plus screen-space collision-nudging and a `path_effects` stroke for readability; **all in-plot text (star/Sol labels + axis tick numbers, axis titles, and ring `N ly` labels) uses `clip_on=True`** (with `annotation_clip=True` on the annotations) so nothing leaks into the black margin when panned/zoomed — only the axes-fraction click-info box stays unclipped; center star drawn as a gold ★ at the origin (Sol for opt 18, queried star for opt 19); **a star flagged `is_sol` also gets a ★** (`_overlay_sol_marker`, ~3× the body-dot size, keeping its paler G-class colour so it stays distinct from the gold centre) — Sol is a synthesized row on opt-19/GCNS charts and would otherwise be an ordinary dot; it is an **overlay artist**, never a per-point marker swap, since the body scatter's point order backs hit-testing/legend-filtering and 3D depth-sorting reorders sizes but not paths; hover tooltip, click info box, scroll-wheel zoom around cursor; `toolbar.push_current()` seeds Home; xlim/ylim callbacks toggle label visibility based on a 15 ly half-range threshold. **Phase I `routes=`** (optional, additive): a list of `{x1,y1,z1,x2,y2,z2,label,style}` edge dicts drawn over the dots — dashed for ordered legs / solid (`_SC_MST`) for MST edges; route lines stay visible at all zooms while the per-segment labels follow the same zoom-driven decluttering as the star labels. Existing opt-18/19 callers pass no `routes` and are unaffected. *(Phase O8 originally shared `routes=` for its two-star edge; since the 2026-07-26 Sol-centered rebuild it draws no edge and passes no `routes`.)* **`label_max_ly=`** (optional, additive) overrides the 15 ly label threshold — the O8 two-star maps pass a large value so their two or three dots stay labelled at any zoom, and (since 2026-07-27) so do the route charts when the route is sparse (≤ 25 nodes; denser ones, e.g. Jump Network, keep the default); `None` keeps 15 ly. `make_star_chart_3d_canvas` takes the same kwarg. |
| `make_star_chart_3d_canvas(parent, stars, limit_ly, routes=None)` | Stars Within Distance 18, 19; Route Planning (Phase I) | 3D companion to `make_star_chart_canvas` (same additive `routes=` overlay): dark navy panes + grid, gold ★ center marker, the same `is_sol` ★ overlay, spectral-class star dots, per-star `"Name (Z=±X.XXX)"` labels **anchored at each star's exact 3D point with left/bottom alignment** (so they track the dot on rotation and zoom instead of drifting on a fixed data-space offset), zoom-driven via `xlim_changed`/`ylim_changed`/`zlim_changed` against `max((x1-x0)/2, (y1-y0)/2, (z1-z0)/2) ≤ 15 ly`; faint wireframe reference spheres at every `major_step` ly out to the limit; **cursor-anchored** hover tooltip (follows the pointer via `_anchor_hover_to_cursor`) + click info box (text2D pinned lower-left); `azel` drag rotation; scroll-wheel zoom via `ax._zoom_data_limits`; rectangle Zoom removed from the toolbar; returns `(canvas, toolbar, ax)` so caller can bind viewpoint preset buttons (Top / Side / 3D Perspective) |
...
| `make_hyper_ring_canvas(parent, data, mode="ghost", classes=None)` | Honorverse Hyper Limits (14) — "Class Rings" / "Class Sectors" | Class-grouped hyper-limit ring diagram (star at centre, **√AU**), the companion to `make_hyper_bar_canvas`. `mode="ghost"` — every subtype a faint solid hairline ring, each class's hottest+coolest drawn bold/dashed and labelled once; `mode="sector"` — the dial split into one wedge per **visible** class, every subtype an **arc at its true radius** confined to its wedge (angle carries no meaning), so F/G/K/M cannot collide. Unknown mode → `"ghost"`. `classes=None` → all groups; otherwise an iterable of group keys, which rescales the dial and (in sector mode) re-divides the circle among the survivors; an empty selection returns a "No spectral classes selected." card, not `(None, None)`. **Uses the dark `_SC_*` Star-Chart surface, not `_SPACE_BG`** — the spectral palette is built for `#0b1020`, and on the light surface F (`#F8F7FF`) and G (`#FFF4EA`) are invisible. Ghost rings are **solid** hairlines (a dotted stroke moirés into a hatch at these radii); the dash is what marks the bold class rings. Click-to-info is wired in ghost mode only — in sector mode a radius-only hit test would name a class the user did not click. Input from `core.viz.prepare_hyper_limit_rings()`. |
...
| `wrap_system_regions_with_hyper_toggle(parent, data)` | Star Regions 8, 9, 13 (10 plain) — Phase O O10b | Wraps the System Regions canvas in a tab; when `data` carries a `hyper_limit` (opts 8/9 with a resolvable spectral type) adds a **"Show Honorverse Hyper Limit (fiction)"** `QCheckBox` (default off) that rebuilds the canvas with `show_hyper=True`; no hyper limit (opt 10) → no checkbox, plain canvas. Mirrors `wrap_orbits_with_solar_toggle`. Returns the container `QWidget` or `None`. |
...
| `OecPanel` (7) | **Data** = tree **+ detail pane** (`QSplitter`, OEC System View 2026-08-02 — see the note above) + a system-level **"Architecture"** map (Phase 3, viz tab 0 — shown for **every** matched system incl. planetless/rogue) + per-host tabs (a **Host** combo when >1 planet-host): "Hypatia", "Orbital Diagram" (+ O4 solar-overlay & O10b hyper-limit), "HZ Diagram", "Mass–Radius" (O3), "Transit Geometry" (O13), "Size Comparison" (O14), "Abundance Profile" + "Kinematics" (O11) (when the host resolves Hypatia). Circumbinary = binary pseudo-host; rogue = Data + Architecture only. Reuses the NASA `_make_*_tab` builders via an OEC→NASA-key adapter (completed_plans/PHASE_OEC_PLAN.md Phase 2). | `DiagramToggleMixin` |
...
| `SolarSystemPanel` (11) | "Orbital Diagram" (combo: Planets / Dwarf Planets + Asteroids), "Moon Systems" (combo: per-planet moons, km secondary axis) — Phase O O7. **Plot cap (2026-08-02):** `prepare_solar_system_orbits` draws one 361-point ellipse per body, so the JPL expansion to 259 asteroids put **264** orbits on the Dwarf-Planets-+-Asteroids view — the 2–3.5 AU main belt became a solid band and the panel was visibly slow. Fixed in the **view**: the prep takes `max_asteroids` (default `_SS_DIAGRAM_MAX_ASTEROIDS` = 25) capping that plot to the largest by diameter **plus every no-diameter body** (the nine curated TNOs — Sedna, Quaoar, Orcus… — would otherwise be silently deleted by a size ranking) plus all 5 dwarf planets → **~39 orbits**. **The data tables are untouched and still list all 259.** The prep returns `asteroids_shown`/`asteroids_total` and this panel appends them to the diagram title, so a capped view never reads as the full catalogue. `max_asteroids=None` disables it; `planets` / `moons:<planet>` ignore it. The combo therefore offers **three** entries — Planets · Dwarf Planets + Asteroids **(major)** · Dwarf Planets + Asteroids **(all)** — the last mapping to `dwarfs_asteroids:all`, so nothing the cap hides is unreachable. Pair it with the scroll-wheel zoom added to `make_orbits_canvas` the same day: at the full extent (Sedna sets ~1180 AU) the main belt needs real magnification to read. Moon Systems is unaffected (≤16 orbits per planet). | `DiagramToggleMixin` |
...
| `HonorverseHyperPanel` (14) | "Class Rings" (default), "Class Sectors", "Hyper Limits" — tab order is deliberate: the two class-grouped ring diagrams (`make_hyper_ring_canvas`, ghost first) precede the original Phase O O10a 44-bar chart, which is unchanged. Each ring tab carries a row of **eight per-class checkboxes** (O·B·A·F·G·K·M·Red Giant, all on) plus **Select All / Clear All** buttons, over an auto-rebuilt canvas — the `solvent_zones._ice_ring_tab` scaffold. The bulk buttons block the per-box signals while setting, so the canvas redraws once rather than eight times; Clear All lands on the canvas's "No spectral classes selected." card. | `DiagramToggleMixin` |
| `StarsWithinDistanceSolPanel` (18) | "Star Chart" (default), "Star Chart 3D", "HR Diagram" (Phase O O2b), "Night Sky" (Phase O O1 — vantage = Sol), "Map X–Y (top-down)", "Map X–Z (edge-on)", "Map 3D"; plus the O18 **Find star** box above the tabs (see "O18 — the Find-Star box" above) | `DiagramToggleMixin` |
| `StarsWithinDistanceStarPanel` (19) | "Star Chart" (default), "Star Chart 3D", "HR Diagram" (Phase O O2b), "Night Sky" (Phase O O1 — mag-limit re-runnable), "Map X–Y (top-down)", "Map X–Z (edge-on)", "Map 3D"; plus the O18 **Find star** box above the tabs |  `DiagramToggleMixin` |
| `DistanceBetweenStarsPanel` (17) | "Star Chart", "Star Chart 3D" (Phase O O8 — **Sol-centered** two-star map with full opt-18/19 parity: per-class legend filter, isochrone control, click-info, 3D presets; O15 row↔map linking on the 2-row table) | `DiagramToggleMixin` |
| `TravelTimeStarsLyHrPanel` (20) / `TravelTimeStarsTimesCPanel` (21) | "Star Chart", "Star Chart 3D" (Phase O O8 — same **Sol-centered** two-star map as opt 17; no row↔map linking, their table is Origin\|Destination-shaped) | `DiagramToggleMixin` |
| **Route Planning** — `MultiStopJourneyPanel` (I1), `NearestNeighborPanel` (I2), `TradeRoutePlannerPanel` (I3), `OptimalTourPanel` (A), `JumpRoutePanel` (B), `JumpNetworkPanel` (C), `FarthestFirstPanel` (D) | "Star Chart", "Star Chart 3D" — the route-overlay maps, re-centred by `_centered` so the route's origin/start sits at the chart origin (gold ★). **Since the route-chart refactor (2026-07-27) both are built by the shared opt-18/19 `_build_iso_chart_tab`**, so they carry the O16 per-class legend filter, the O17 isochrone control (rings = travel time *from the start*), the click-info box and the 3D viewpoint presets; the dashed-leg / solid-MST overlay rides along on that builder's `routes=`. Two exceptions: `JumpNetworkPanel` passes `legend_filter=False` (its dots are per-tier coloured, so a class-grouped legend would mislabel them — its own tier-swatch legend above the table is the key), and `label_max_ly` is raised only for sparse routes (≤ 25 nodes). O15 row↔map linking on the three star-per-row panels (Nearest-Neighbor, Farthest-First, Jump Network) via `name_col=1` — those tables lead with an index column; the leg-shaped `From\|To` panels pass no `link_view`. **The O18 Find-star box (2026-07-31) is on all seven**, sourced from the route star list rather than the result table — that is what covers the four leg-shaped panels — with the start ★ excluded and the ring applied directly to each canvas rather than via table selection; Jump Route's `reachable=False` chart passes `find_box=False` **only when it has no waypoints** — with `via` stars the failed chart carries k+2 terminals, which is something to find. See "O18 — the Find-Star box" above; tests `tests/test_route_find.py`. **Panel descriptions (2026-07-27):** each of the seven carries a `DESCRIPTION` class attribute (rich text) rendered by `_build_description_box` as a hidden QLabel at the **top of `_tables_widget`** — i.e. in the results pane, directly above the run's data — toggled by a **Show/Hide Description** button that `_button_row` places between Run and Show Diagrams. It is a persistent widget: `_clear_tables_layout` deletes from layout index `panel._tables_keep` **onward** (not from 0), so a Run (or an error render) never destroys it and the toggle stays in sync. The toggle reads `isHidden()`, not `isVisible()` (an ancestor hidden in diagram mode would desync the latter). Tests: `tests/test_route_chart_tabs.py::RouteDescriptionTest`. **`JumpRoutePanel` waypoints (2026-07-31):** a **"Via (optional, comma-separated)"** `QLineEdit` sits between Destination and Max Jump (in `enter_fields`); it is split on commas, blanks dropped, and passed as `compute_jump_route`'s 5th positional — **an empty field is the off switch**, there is no toggle, and **Optimize For** already supplies the objective the waypoint ordering minimizes. A reachable waypointed route gets a visit-order line above the table (`Via: A → B`, plus a note when the route re-visits a star) and a **◆** prefix on the To cell of each waypoint-arrival row (driven by `route[i]["waypoint"]`, i.e. by route index — not by name match). The unreachable branch renders `unreachable_leg` rather than the hardcoded origin→destination pair, since with waypoints the failed hop is often `origin → waypoint`. Tests: `tests/test_route_chart_tabs.py::JumpRouteViaPanelTest` | `DiagramToggleMixin` |
...
| `prepare_solar_system_orbits(kind="planets", max_asteroids=25)` | **Phase O O7.** Solar-system orbital-ellipse data from `core.science.compute_solar_system_tables()`. `kind ∈ {"planets", "dwarfs_asteroids", "dwarfs_asteroids:all", "moons:<planet>"}` — the `:all` variant forces `max_asteroids=None` (see the `SolarSystemPanel` plot-cap note) — moon SMAs are km (÷1.496e8 → AU). Same orbit-dict shape as `prepare_system_orbits` plus `{hz_zones: [], max_au, star_name}` (no HZ) or `{"error": str}`. Feeds `make_orbits_canvas` (opt 11). |
| `prepare_hz_diagram(teff, luminosity)` | Returns `{"zones": list, "max_au": float}` or `{"error": str}`. Each zone dict: `key, label, outer (AU), color`. |
| `prepare_star_map_from_result(result)` | Converts `compute_stars_within_distance_of_sol/star` result dict to star-map format. Center star placed at origin; surrounding stars' coordinates shifted accordingly. Each star carries an additive **`is_sol`** flag (2026-07-31) that the chart canvases paint as a ★ — set on the synthesized Sol row of a star-centred (opt-19) result, and never on a Sol-centred (opt-18) one, where Sol is already the gold centre ★. |
| `prepare_system_regions_diagram(d)` | Extracts seven labelled boundary AU values + Kopparapu HZ zones + EEID from a star-regions result dict. Returns `{"regions", "hz_zones", "eeid_au", "max_au"}`. **Phase O O10b:** when `d["spectral_type"]` resolves (opts 8/9), also attaches a separate `"hyper_limit": {label, au, color, matched_class}` overlay key (via `core.science.compute_hyper_limit_for_spectral_type`, ceiling rule) — **not** part of `regions`; the canvas draws it only with `show_hyper=True`. Absent for opt-10 dicts (no `spectral_type`) or an unresolvable type; opt 13 carries `spectral_type="G2V"` (`compute_sol_regions`), so it gets one. |
...
| `prepare_hyper_limit_rings()` | Class-grouped companion to `prepare_hyper_limits` — a **regrouping** of the same 44 rows into the eight display groups O·B·A·F·G·K·M·**RG** (catalogue order, hot→cool), so a ring diagram carries 8 labels instead of 44. Returns `{"groups": [{key, label, color, rows, lo_au, hi_au, lo_class, hi_class}], min_au, max_au}`; no row is dropped (a group's `rows` holds every subtype). "Red Giant" is its own group and keeps `sp_color`'s **unknown grey `#AAAAAA`** — it is not a spectral class, and inventing a hue here would add a second palette (forbidden by `tests/test_search.py::test_there_is_exactly_one_spectral_palette`). Feeds `make_hyper_ring_canvas`. |
...
| `prepare_sky_from_star(result, mag_limit=6.5)` | **Phase O O1.** Converts a stars-within result into night-sky data for `make_sky_canvas()`. Vantage = the queried centre star (opt 19) or **Sol at the origin** (opt 18, the Sol-centric result with no `center`). Each star's vector from the vantage → RA/Dec; apparent magnitude from the vantage via the distance modulus. **Sol is no longer appended here** (2026-07-31) — it arrives as an ordinary star row synthesized by `compute_stars_within_distance_of_star`, whose `app_magnitude`/`parsecs` recover the same M_V 4.83 this used to hard-code; re-adding the special case would put Sol on the sky twice. NULL-V-mag stars are counted in `skipped_no_mag` (never fabricated). Returns `{"vantage_name", "mag_limit", "skipped_no_mag", "stars":[{name, ra_deg, dec_deg, mag, sp_class, color}]}` or `{"error": str}`. Needs the F1 `app_magnitude`/`parsecs` row keys. |
...
| D | Complete | Multi-source features, JPL Horizons, option 50 (opts 3–6, 26–32, 50); opt 2 implemented but not in GUI nav (opt 7 likewise until the Phase OEC rebuild added `OecPanel`) |

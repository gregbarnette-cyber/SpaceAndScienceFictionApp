...
gui/                 # Qt presentation layer
  ...
  panels/
    ...
    diagram_tabs.py      # shared stateless diagram-tab builders (_make_{hz,orbits,mass_radius,transit,size}_tab)
                         ...
                         #   the O15 row↔map link helpers, moved here from distance_stars.py so the two-star panels
                         #   (opts 17/20/21, via route_planning.add_two_star_chart_tabs) reuse the exact opt-18/19
                         #   builders without a circular panel import.
...
> remains a **separate palette** with the same prefix-aware derivation and additive `L/T/W/Y/C/N` keys — unification
> is deferred to `ROUTE_CHART_REFACTOR_PLAN.md`. `_SPECTRAL_COLORS` is physically motivated and deliberately fails

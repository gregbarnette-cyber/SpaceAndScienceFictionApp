...
- **Lifecycle.** `_add_find_box` adds nothing when the searchable set is empty, so an empty
  Nearest-Neighbor/Farthest-First chain and Jump Route's `reachable=False` two-endpoint chart
  (`find_box=False`) get no box — the same "nothing to find" reasoning that keeps it off opts
  17/20/21. `_prepare_render` (`base.py`) clears `_find_rows`/`_find_matches`/`_find_idx`

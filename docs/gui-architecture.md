...
| post-F | Complete | **GCNS** (Gaia Catalogue of Nearby Stars): opt 58 `ImportGcnsPanel` / `import_gcns_data` ingests ~331k sources into the isolated `gcns_stars` table (+ `gcns_meta`) via GAVO TAP; exposed only through `query.py` (`gcns-within-sol`, `gcns-source`) — no existing option displays it; `get_table_status()` lists GCNS Stars + GCNS Meta. See `docs/star-databases.md` (ingest) and `docs/integration.md` (query contract). |

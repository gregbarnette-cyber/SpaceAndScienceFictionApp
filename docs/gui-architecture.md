...
| `prepare_abundance_profile(hypatia_result)` | Converts a `compute_hypatia_data` result into bar-chart data for `make_abundance_canvas()`. Returns `{"elements", "means", "stds", "star_name"}` or `{"error": str}`; filters to elements with a non-None mean, preserving `_HYPATIA_ELEMENTS` order. |

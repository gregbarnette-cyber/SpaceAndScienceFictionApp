...
| `wrap_orbits_with_solar_toggle(parent, build_canvas)` | NASA opts 3, 6, Map panel — Phase O O4 | Wraps an orbital-diagram canvas in a tab with a "Show Solar System reference" `QCheckBox`; `build_canvas(solar_overlay: bool) -> (canvas, toolbar)` is rebuilt in place on toggle. Default unchecked → the canvas is byte-identical to before. Returns the container `QWidget`, or `None` if the canvas can't build. |
...
| `NasaPlanetarySystemsPanel` (3) | "Orbital Diagram" (+ Phase O O4 solar-overlay checkbox), "HZ Diagram", "Mass–Radius" (Phase O O3), "Transit Geometry" (Phase O O13), "Size Comparison" (Phase O O14), "Abundance Profile" (when Hypatia data available) | Inline (uses `_scroll_area`) |
| `NasaPlanetarySystemsMapPanel` | "System Map", "Orbital Diagram" (+ O4 solar-overlay checkbox), "HZ Diagram", "Mass–Radius" (O3), "Transit Geometry" (O13), "Size Comparison" (O14), "Abundance Profile" (when Hypatia data available) | Inline (uses `_scroll_area`) |
...
| `HwcPanel` (6) | "Orbital Diagram" (+ Phase O O4 solar-overlay checkbox), "HZ Diagram" (lum = `S_LUMINOSITY` direct Lsun), "Mass–Radius" (Phase O O3), "Size Comparison" (Phase O O14), "Abundance Profile" (when Hypatia data available) | `DiagramToggleMixin` |

...
core/                # Pure computation layer — no I/O, no Qt
  ...
  priors.py          # Synthetic-realism priors (Phase R1): DefaultPriors (the R3 seam)
...
| `SystemGeneratorPanel` (R1/R2) | "Orbit Diagram" (`make_orbits_canvas`, observed-green vs synthetic-blue orbits + HZ annulus + snow-line ring), "HZ Ring" (`make_hz_canvas`); **Phase R2** adds an in-place constraint builder (`_ConstraintRow`) + a four-layer feasibility banner/cards (verdict chip, Layer-1 reason, Layer-2 mechanism, Layer-3 tagged origin, Layer-4 clickable-apply alternative buttons) when ≥1 constraint is present | `DiagramToggleMixin` |

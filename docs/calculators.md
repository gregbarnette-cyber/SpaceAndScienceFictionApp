...
- **GUI diagram tabs** (via `DiagramToggleMixin`), in display order: "Star Chart" (the **default** selected tab when Show Diagrams is pressed), "Star Chart 3D", "HR Diagram", "Night Sky", "Map X–Y (top-down)", "Map X–Z (edge-on)", "Map 3D". The three Map tabs use a light gray background (`bg="#ebebeb"`). The 3D tab includes Top View / Side View / 3D Perspective preset buttons above the matplotlib toolbar. Stars are coloured by spectral class; Sol is highlighted with a star marker at the origin. Hover shows name + distance in a **cursor-anchored** tooltip (it follows the pointer next to the hovered dot, clamped on-canvas); click shows full info box. Scroll wheel zooms in/out; Home button resets to the initial zoom and view angles. The rectangle Zoom button is removed from the 3D toolbar — it cannot map a 2D screen selection back to 3D data coordinates correctly.
  ...
  - **Wikipedia article tab (row-click, GUI-only).** The table view is now a small `QTabWidget` — a permanent **"Results"** tab (the count label + table) plus closable **"📖 ‹star› — Wikipedia"** tabs — with a row-selection **"📖 Open in Wikipedia →"** button below it that opens the selected star's Wikipedia article (fetched lazily, rendered as text; `core/wikipedia.py` + `gui/panels/wikipedia_tab.py`). The existing `_tables_layout` is kept as the Results tab's inner layout so the render code is unchanged, and `_clear_tables_layout` closes the wiki tabs only when `_results_tabs` exists — **opt 17 is untouched**. Show Diagrams, the star charts, the Find-star box and O15 row↔map linking all keep working. See `docs/star-databases.md` (Wikipedia Article Tab) and `docs/gui-architecture.md`. Tests: `tests/test_wikipedia_panels.py`.
...
- **Malformed-row tolerance**: opt 50 writes `ra`/`dec` as `""` when SIMBAD's value fails to parse, and the split-based sexagesimal parsers raise **`IndexError`** (not `ValueError`) on a blank or short string. The per-row handler therefore catches `(ValueError, TypeError, IndexError)` and skips the row (the core/GUI/`query.py` path; the deprecated `main.py` CLI copy still catches only `(ValueError, TypeError)`) — matching `_load_star_systems_positions`. Before this, a single degenerate row aborted the entire search; it was masked only because the opt-50 `PLX …` discard rule happened to filter most such rows out. Opt 18 already caught bare `Exception` (it returns the row with `x/y/z = None`, which every viz prep skips).
...
- **CR-18 — transverse separation + bound flag (`query.py stars-within-star`, additive; 2026-09-01).** Each neighbour in the `query.py` result gains `transverse_sep_au`/`transverse_sep_ly` (plane-of-sky separation — the honest physical separation the parallax-noise-dominated 3D `Distance` overstates), a tri-state `bound` (`true`=GCNS-bound / `false`=GCNS-optical / **`null`=outside the GCNS layer, "unknown" NOT "unbound"**), `is_bound_companion`, `sep_method` (`gcns_proj_sep` for a GCNS-co-systemed neighbour, else `computed_angular`), and `radial_parallax_dominated`. Transverse uses the centre's SIMBAD distance (always available); the bound flag needs the centre's Gaia id → GCNS system: parsed offline from the centre's own `star_systems` designations row (matched on the SIMBAD `main_id`), falling back to one `compute_simbad_lookup` only when that misses — all gated to a non-Sol centre + a populated `gcns_stars`. The GUI tables are unchanged (additive `query.py` keys). See `docs/integration.md` (CR-18 block).
...
- Only includes units that are ≥ 1 (seconds are shown only when the remaining seconds are ≥ 0.005 **and** no larger unit applies or the total is < 1 minute; returns `"0 Seconds"` if nothing applies).
- Uses Julian year: `HOURS_PER_YEAR = 365.25 × 24 = 8766.0` (not the 8765.8128 velocity anchor above), `HOURS_PER_MONTH = HOURS_PER_YEAR / 12`.
...
- **`compute_lookup_star_for_distance`** — uses `_make_simbad("plx_value", "sp_type")` (default 30 s timeout) + `_with_retries` + `_timeout_ctx(30)`, same pattern as SIMBAD callers in `databases.py`. Error messages classified via `_network_error_msg`.
...
- Uses `_resolve_horizons_id()` and `_HORIZONS_ID_MAP` (same as option 22).
- **Iterative destination position estimation**: unlike option 22 which uses a single snapshot, this function queries the destination's position at the estimated arrival time and iterates until the travel time converges (change < 60 seconds, max 10 iterations). Origin position is fixed at departure epoch. Uses `_get_heliocentric_vectors()` with `epoch_jd` parameter.
...
- Same error handling as option 22: ambiguous Horizons name, lookup failure, same-object detection (distance < 1e-9 AU).
...
`core/dust_routing.py` (`compute_*_dust`), surfaced by the `query.py` `--weight {distance,dust}` flag (`jump-route` additionally accepts `blend` + `--alpha`/`--beta` → `compute_jump_route_blend`) (default
...
(tour/multi-stop/nearest-neighbor/MST — four forks) take no `via`: waypoints are a `jump-route` concept, since those
